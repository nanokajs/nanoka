...
Phase 1（M0〜M8）の完了後、Phase 2（リレーション / フィールドアクセサ / OpenAPI 等）に着手する前に、**公開ライブラリとしての運用基盤・型改善・onboarding 体験の穴**を埋めるためのフェーズ。`docs/backlog.md` の §1（Phase 1 内の持ち越し）と §4（運用・インフラ）をマイルストーンに整理したもの。
...
- [ ] **1.1** `Nanoka<E extends Env = BlankEnv> extends Hono<E>` 対応
...
- [ ] **1.2** `examples/basic/test/e2e.test.ts` の seed を `User.create` 経由に書き換え
...
- [ ] **4.5** `packages/nanoka/README.md` の `../../examples/basic` 相対リンクを GitHub 絶対 URL に置換
  - 置換先: `https://github.com/nanokajs/nanoka/blob/main/examples/basic/README.md`
  - 動機: npm registry 上では相対リンクが解決されない
- [ ] **4.3** `CONTRIBUTING.md` を新規作成（リポジトリルート）
...
- [ ] **4.6** `docs/phase1-plan.md` を完了履歴アーカイブとして整理（任意・スキップ可）
...
- [ ] **4.1a** GitHub Actions: PR / push to main で build / test / typecheck / lint を回す
...
- [ ] **4.4** LICENSE 二重配置の diff チェックを CI に組み込む
...
完了基準: PR 作成時に CI が走り、build/test/typecheck/lint と LICENSE 同期チェックが通る。
