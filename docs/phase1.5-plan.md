...
- [x] **4.7** README どおりの最小プロジェクトを CI で組み立て、`tsc --noEmit` と `drizzle-kit generate` を回す E2E
  - 手順:
...
- [x] **4.1b** tag push (`v*`) で `pnpm publish` を自動化する GitHub Actions
...
決定した認証方式: npm Trusted Publisher (OIDC)。`NPM_TOKEN` は不使用。
