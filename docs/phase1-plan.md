...
- [x] `repository` / `homepage` / `bugs`: GitHub URL は **未確定なので publish 直前に追記** とする旨をコメントアウトせず JSON では空のまま、README 末尾の Release checklist に明記
...
- [x] `repository` / `homepage` / `bugs` フィールドを GitHub URL で埋める
