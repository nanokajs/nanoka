...
- **パッケージ名は `@nanokajs/core`**（`nanoka` は名前類似性で npm にブロックされたため、`nanokajs` org 下の scope 付きパッケージとして publish する方針に決定）
...
- [x] `repository` / `homepage` / `bugs`: URL 確定済み（`https://github.com/nanokajs/nanoka.git`、新 org `nanokajs` 配下に移行）。M8 で package.json に追加済み
...
- [x] npm パッケージ名を `@nanokajs/core` で publish 確定（`nanoka` は名前類似性で npm にブロックされたため、`nanokajs` org を作成して scope 付きに変更）。`npm view @nanokajs/core` で E404 になること、および `nanokajs` org に publish 権限があることを確認
