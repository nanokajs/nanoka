# Phase 2後半 / Phase 3 混在実装整理

Claude Code の作業により、当初 Phase 2 後半候補だった Turso/libSQL adapter と、Phase 3 候補だった route-level OpenAPI / Swagger UI / CLI スキャフォールダが同じ流れで実装された。本ファイルは、その混在実装を追認し、何が入っていて何が残っているかを整理するための履歴ドキュメントとする。
...
1. OpenAPI spec を route metadata から生成できる
2. Swagger UI で生成 spec を閲覧できる
3. Turso/libSQL を D1 と同じ adapter interface で使える
4. `create-nanoka-app` で最小 Nanoka app を生成できる
...
- [x] **1.1** OpenAPI ルートメタデータの明示登録
  - `app.openapi(metadata)` API の設計と実装
  - 自動走査ではなく手動アノテーション方式を採用（80%自動・20%明示の設計方針と整合）
  - Nanoka 経由で登録した GET / POST / PATCH / DELETE ルートに `app.openapi()` でメタデータを付与し、path / method / requestBody / responses を収集する
- [x] **1.2** route-level OpenAPI spec 生成エンドポイント
...
- [x] **1.4** 未対応 Zod 型の処理決定（`docs/backlog.md §4.11` 持ち越し）
...
完了基準: `pnpm create nanoka-app my-app` でプロジェクトが生成される。生成されたプロジェクトで `tsc --noEmit` と `drizzle-kit generate` が成功する。onboarding parity CI が scaffold 産出物に対して green。`create-nanoka-app` が npm registry に publish 可能な package として整備される。
