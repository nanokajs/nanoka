---
name: phase2b-next
description: Phase 2後半 (M1〜M3) を 1 マイルストーンずつ逐次的に進めるオーケストレーション。`docs/phase2後半-plan.md` を読んで次の未完了マイルストーンを特定し、planner → implementer → implementation-reviewer → security-reviewer の順にサブエージェントを呼び、各ゲートでユーザー確認を取る。1 回の起動で **1 マイルストーンだけ** 進める（M1 をやったら M2 には自動で進まない）。
---

# Phase 2後半 を 1 マイルストーンずつ進める

このスキルは **1 マイルストーンを完了させたら止まる**。「次の M に進んでいいか」はユーザーが明示的に判断する。

Phase 2 前半（M1〜M4）でリレーション・フィールドアクセサ等を実装した後、OpenAPI生成・Turso/libSQLアダプタ・CLIスキャフォールダ等を実装するフェーズ。`docs/nanoka.md` の Phase 3 候補と `docs/backlog.md` §3.5 / §4.10 / §4.11 をマイルストーン化したもの。

すべてのマイルストーンが独立で並行可。ただし M1 は Phase 2C（M3）で整備した `packages/nanoka/src/openapi/generate.ts` の API が安定していることを着手前に確認すること。

## 手順

### Step 1: 状況把握
1. `docs/phase2後半-plan.md` を Read で読み、すべてのマイルストーン（`### M1:` 〜 `### M3:`）とそのチェックボックス状態を確認する。
2. **次に着手すべきマイルストーン** を特定する:
   - M1 から順に見て、配下のチェックボックスのうち 1 つでも未完了 (`- [ ]`) があるマイルストーンが対象。
   - すべてのマイルストーンが独立で並行可。ただし M1 は Phase 2C（M3）で整備した `packages/nanoka/src/openapi/generate.ts` の API が安定していることを着手前に確認すること。
   - すべてのマイルストーンが完了済みなら「Phase 2後半 完了」と報告して終了。
...
1. `Agent` ツールで `subagent_type: "planner"` を呼び出す。プロンプトには以下を含める:
   ...
   - Phase 3 機能（Relations / 型安全クエリビルダー / VSCode 拡張 / Claude Code プラグイン）および受容リスクは本フェーズ対象外。`docs/backlog.md` §2 / §3.5 / §4.10 の管轄。
...
Phase 2後半 のマイルストーン別の目安:
- M1 (OpenAPI生成): **sonnet** — Hono ルート走査・JSON Schema 変換・型統合が複雑
- M2 (Turso/libSQLアダプタ): **sonnet** — adapter interface の型制約・新パッケージ横断
- M3 (CLIスキャフォールダ): **haiku** — ファイル生成・テンプレート展開は機械的
...
1. `Agent` ツールで `subagent_type: "implementer"` または `subagent_type: "implementer-sonnet"` を呼ぶ。プロンプトには以下を含める:
   ...
   - 「サブタスク完了ごとに `docs/phase2後半-plan.md` の該当チェックボックスを更新すること」を明示
...
1. `Agent` ツールで `subagent_type: "implementation-reviewer"` を呼ぶ。プロンプトには:
   ...
   - 参照すべきドキュメント: `docs/nanoka.md` / `docs/phase2後半-plan.md` / `docs/backlog.md`
2. レビュー結果をユーザーに見せる。
...
すべてのマイルストーンでセキュリティレビューを実施する:
- **M1**: OpenAPI spec を public endpoint で公開するため、フィールドポリシー（serverOnly / writeOnly）の漏洩リスクを確認
- **M2**: 外部 DB 接続（Turso）の credential 取り扱いと DB クエリパスを確認
- **M3**: CLI ファイル生成における path traversal リスクと CI/CD pipeline への影響を確認
...
### Step 6: 完了処理
1. `docs/phase2後半-plan.md` の該当マイルストーンのチェックボックスがすべて埋まっていることを確認。埋まっていなければ更新。
...
- **`docs/phase2後半-plan.md` の更新タイミングを守る**: implementer がサブタスク単位で更新する。オーケストレータが事前に勝手にチェックを入れない。
- **依存関係を尊重する**: すべてのマイルストーンが独立で並行可。ただし M1 は Phase 2C（M3）で整備した `packages/nanoka/src/openapi/generate.ts` の API が安定していることを着手前に確認すること。
- **Phase 3 機能（Relations / 型安全クエリビルダー / VSCode 拡張 / Claude Code プラグイン）および受容リスクは本フェーズ対象外。`docs/backlog.md` §2 / §3.5 / §4.10 の管轄。**
- 途中でエラー・矛盾・スコープ膨張を検知したら、自動で続行せず **ユーザーに判断を仰ぐ**。

## 引数

引数で特定マイルストーンを指定された場合（例: `/phase2b-next M3`）は、Step 1 の自動判定を上書きしてそのマイルストーンを対象にする。ただし依存マイルストーンが未完了なら警告を出してユーザー確認。
